...
public class YBotSimpleControlScriptJessica : MonoBehaviour
{
   ...
   // bool isFalling = false;
    ...
    void Awake()
    {
        ...
        collectText = collectibleText.GetComponent<Text>();
        hitText = hitsText.GetComponent<Text>();
    ...
    void OnCollisionEnter(Collision collision)
    {
		...
		if (collision.transform.gameObject.tag == "Ball") {
             ...
             collision.transform.gameObject.GetComponent<SphereCollider>().enabled = false;
            
             EventManager.TriggerEvent<CollectibleEvent, Vector3>(collision.contacts[0].point);
            Destroy(collision.transform.gameObject);
            collectText.text = "Collected: " + ballCount.ToString() + "/3";
		}
        if (collision.transform.gameObject.tag == "enemy") {
            hits++;
            hitText.text = "Hits: " + hits.ToString() + "/3";
            EventManager.TriggerEvent<EnemyHitEvent, Vector3>(collision.contacts[0].point);
        }
    }
    void OnTriggerEnter(Collider collision)
    {
        if (collision.transform.name == "endLevel") {
